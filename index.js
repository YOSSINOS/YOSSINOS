...
const devs = ["959783034498998293","707931169597030471","738551496236597378","850022889369632818"]
setTimeout(() => {
...
client.on("message",async message =>{
  ...
  .setColor("BLUE").setAuthor(message.author.username , message.author.displayAvatarURL({dynamic:true})).setTimestamp().setFooter("Hollywood", message.guild.iconURL()).setTitle("**Hollywood luck spins**").addField("Spin Normel" , "تحتاج الى invites واحد للحصول على نقطه.").addField("Legend spin","تحتاج إلى invites اثنين للحصول على نقطتين.")
...
collector.on('collect',b =>{
  if(b.id == `cancel_${message.author.id}`){
    ...
    let rewn = ["1OK","20k","30k","40K","50K","60K","70K","80K","Retry"]
...
db.subtract(`${message.author.id}.spins`,2)
   ...
   let rewn = ["60k","75k","80k","90k","100k","150K"]
  let praise = rewn[Math.floor(Math.random() * rewn.length)];
 message.lineReplyNoMention(`Congratulat you won ${praise}`)
...
client.on("ready",()=>{
client.user.setActivity("Hollywood Luck.", { type: 'WATCHING' })
